...
app.patch('/api/members/:id', async (req, res) => {
  try {
    ...
    [
      'apc_file_name','apc_file_type','apc_file_data',
      'passport_file_name','passport_file_type','passport_file_data',
      'mission_document_name','mission_document_type','mission_document_data',
      'lampiran_ketua_jabatan_name','lampiran_ketua_jabatan_type','lampiran_ketua_jabatan_data'
    ].forEach(field => {
      if (updates[field] === undefined || updates[field] === null || updates[field] === '') delete updates[field];
    });
...
let memObservations = [];
let memReferrals = [];
let memDispensaryRecords = [];
let memMedicineInventory = [];
let memIliRecords = [];
let memSariRecords = [];
...
async function getDispensaryRecords(){if(!USE_S3)return memDispensaryRecords;return await s3Get('myemt/dispensary-records.json')||[];}
async function saveDispensaryRecords(list){if(!USE_S3){memDispensaryRecords=list;return;}await s3Put('myemt/dispensary-records.json',list);}
async function getMedicineInventory(){if(!USE_S3)return memMedicineInventory;return await s3Get('myemt/medicine-inventory.json')||[];}
async function saveMedicineInventory(list){if(!USE_S3){memMedicineInventory=list;return;}await s3Put('myemt/medicine-inventory.json',list);}
...
app.post('/api/sessions', async (req,res) => {
  ...
  const now=new Date().toISOString(); const item={id:makeId('S'),name:req.body.name.trim(),level:req.body.level,startDate:req.body.startDate,endDate:req.body.endDate,location:req.body.location.trim(),latitude:Number.isFinite(Number(req.body.latitude))?Number(req.body.latitude):null,longitude:Number.isFinite(Number(req.body.longitude))?Number(req.body.longitude):null,attendanceRadiusM:Math.max(50,Number(req.body.attendanceRadiusM)||300),quota,status:req.body.status,objective:req.body.objective||'',components:COURSE_COMPONENTS,tentative:Array.isArray(req.body.tentative)?req.body.tentative:[],createdAt:now,updatedAt:now};
...
app.patch('/api/mds/:id',async(req,res)=>{const list=await getMds(),i=list.findIndex(x=>String(x.id)===String(req.params.id));if(i<0)return res.status(404).json({success:false,message:'Rekod MDS tidak dijumpai.'});const{id,createdAt,missionId,mdsRecordId,...updates}=req.body||{},merged={...list[i],...updates,missionId:list[i].missionId},problem=validateMdsContext(merged);if(problem)return res.status(400).json({success:false,message:problem});if(!cleanText(merged.patientName,200))return res.status(400).json({success:false,message:'Nama pesakit diperlukan.'});const stages=['registered','doctor_completed','pharmacy_completed','command_post_verified'],currentStage=stages.includes(list[i].workflowStage)?list[i].workflowStage:'registered',requestedStage=updates.workflowStage||currentStage;if(!stages.includes(requestedStage))return res.status(400).json({success:false,message:'Peringkat aliran kerja tidak sah.'});if(stages.indexOf(requestedStage)>stages.indexOf(currentStage)+1)return res.status(409).json({success:false,message:'Lengkapkan setiap peringkat MDS mengikut turutan: pendaftaran, doktor, farmasi dan Pos Kawalan.'});const stage=stages.indexOf(requestedStage)<stages.indexOf(currentStage)?currentStage:requestedStage;const prescribedMedications=Array.isArray(list[i].medications)?list[i].medications:(Array.isArray(merged.medications)?merged.medications:[]);if(stage==='pharmacy_completed'&&currentStage!=='pharmacy_completed'&&prescribedMedications.length){const dispensed=(await getDispensaryRecords()).some(item=>String(item.missionId)===String(merged.missionId)&&String(item.mdsId)===String(list[i].id||list[i].mdsRecordId));if(!dispensed)return res.status(409).json({success:false,message:'Lengkapkan serahan ubat melalui modul Dispensari Ubat supaya rekod pendispensan disimpan dan stok ditolak.'});}if(stage==='command_post_verified'&&merged.finalConfirmed!==true)return res.status(400).json({success:false,message:'Pengesahan akhir Pos Kawalan diperlukan.'});const now=new Date().toISOString(),status={registered:'Pendaftaran Selesai',doctor_completed:'Konsultasi Doktor Selesai',pharmacy_completed:'Farmasi Selesai',command_post_verified:'Disahkan Pos Kawalan'},activityDate=merged.activityDate||merged.date,activityLocationId=cleanText(merged.activityLocationId||merged.activityLocation?.id,150);list[i]={...merged,id:list[i].id,mdsRecordId:list[i].mdsRecordId||list[i].id,missionId:list[i].missionId,createdAt:list[i].createdAt,date:activityDate,activityDate,activityLocationId,mdsCodes:normalizeMdsCodes(merged.mdsCodes),updatedAt:now,workflowStage:stage,recordStatus:status[stage]};await saveMds(list);res.json({success:true,message:'Rekod MDS berjaya dikemas kini.',data:list[i]});});

async function validateClinicalMdsLink(body){const missionId=cleanText(body.missionId,100),mdsId=cleanText(body.mdsId,100);if(!mdsId)return 'ID MDS diperlukan.';return (await getMds()).some(item=>String(item.id)===mdsId&&String(item.missionId)===missionId)?'':'Rekod MDS tidak sepadan dengan misi.';}
...
app.post('/api/mission-reports',async(req,res)=>{const missionId=cleanText(req.body.missionId,100),type=cleanText(req.body.type,30),fields=req.body.fields&&typeof req.body.fields==='object'&&!Array.isArray(req.body.fields)?req.body.fields:null,status=String(req.body.status||'DRAFT').toUpperCase();if(!missionId||!['exit','lessons'].includes(type)||!fields)return res.status(400).json({success:false,message:'ID misi, jenis laporan dan kandungan berstruktur yang sah diperlukan.'});if(!['DRAFT','SUBMITTED'].includes(status))return res.status(400).json({success:false,message:'Status laporan tidak sah.'});if(!(await getMissions()).some(x=>String(x.id)===String(missionId)))return res.status(404).json({success:false,message:'Misi tidak dijumpai.'});const required=type==='exit'?['incidentName','teamName','deploymentStart','deploymentEnd','preparedBy','reportDate']:['incidentName','teamName','deploymentStart','deploymentEnd','goodPractices','preparedBy','reportDate'],complete=required.filter(key=>cleanText(fields[key],500)).length,completeness=Math.round(complete/required.length*100);if(status==='SUBMITTED'&&completeness<100)return res.status(400).json({success:false,message:`Laporan belum lengkap. Lengkapkan: ${required.filter(key=>!cleanText(fields[key],500)).join(', ')}.`});const list=await getMissionReports(),now=new Date().toISOString(),index=list.findIndex(x=>String(x.missionId)===missionId&&x.type===type),history=index>=0?(Array.isArray(list[index].statusHistory)?list[index].statusHistory:[]):[],payload={missionId,type,fields,status,completeness,submittedAt:status==='SUBMITTED'?(list[index]?.submittedAt||now):null,updatedAt:now,statusHistory:[...history,{status,changedAt:now}]};if(index>=0){list[index]={...list[index],...payload,id:list[index].id,createdAt:list[index].createdAt};}else{list.push({...payload,id:makeId(type==='exit'?'EXT':'LLR'),createdAt:now});}await saveMissionReports(list);const item=index>=0?list[index]:list[list.length-1];res.json({success:true,message:status==='SUBMITTED'?'Laporan misi berjaya dihantar.':'Draf laporan berjaya disimpan.',data:item});});

function registerClinicalResource(path,getList,saveList,prefix,label){app.get(`/api/${path}`,async(req,res)=>{let list=await getList();if(req.query.missionId)list=list.filter(item=>String(item.missionId)===String(req.query.missionId));res.json({success:true,data:list,total:list.length});});app.get(`/api/${path}/:id`,async(req,res)=>{const item=(await getList()).find(value=>String(value.id)===String(req.params.id));item?res.json({success:true,data:item}):res.status(404).json({success:false,message:`Rekod ${label} tidak dijumpai.`});});app.post(`/api/${path}`,async(req,res)=>{const missionId=cleanText(req.body.missionId,100),patientName=cleanText(req.body.patientName,200),date=req.body.observationDate||req.body.referralDate;if(!missionId||!patientName||!date||!validDate(date))return res.status(400).json({success:false,message:'ID misi, nama pesakit dan tarikh yang sah diperlukan.'});if(!(await getMissions()).some(item=>String(item.id)===missionId))return res.status(404).json({success:false,message:'Misi tidak dijumpai.'});const linkError=await validateClinicalMdsLink(req.body);if(linkError)return res.status(409).json({success:false,message:linkError});const list=await getList(),now=new Date().toISOString(),item={...req.body,id:makeId(prefix),missionId,patientName,createdAt:now,updatedAt:now};list.push(item);await saveList(list);res.json({success:true,message:`Rekod ${label} berjaya disimpan.`,data:item});});app.patch(`/api/${path}/:id`,async(req,res)=>{const list=await getList(),index=list.findIndex(item=>String(item.id)===String(req.params.id));if(index<0)return res.status(404).json({success:false,message:`Rekod ${label} tidak dijumpai.`});const{id,missionId,createdAt,...updates}=req.body||{},candidate={...list[index],...updates,missionId:list[index].missionId};const date=updates.observationDate||updates.referralDate;if(date&&!validDate(date))return res.status(400).json({success:false,message:'Tarikh tidak sah.'});if(updates.patientName!==undefined&&!cleanText(updates.patientName,200))return res.status(400).json({success:false,message:'Nama pesakit diperlukan.'});const linkError=await validateClinicalMdsLink(candidate);if(linkError)return res.status(409).json({success:false,message:linkError});list[index]={...candidate,id:list[index].id,missionId:list[index].missionId,createdAt:list[index].createdAt,updatedAt:new Date().toISOString()};await saveList(list);res.json({success:true,message:`Rekod ${label} berjaya dikemas kini.`,data:list[index]});});}
...
function dispensaryValidation(body){const status=cleanText(body.status,50),items=Array.isArray(body.prescriptions)?body.prescriptions:[];if(!body.dispenseDate||!validDate(body.dispenseDate))return 'Tarikh pendispensan yang sah diperlukan.';if(!cleanText(body.dispensaryLocation,300))return 'Lokasi dispensari diperlukan.';if(!cleanText(body.officerName,200))return 'Nama pegawai yang mendispens diperlukan.';if(!['Lengkap','Sebahagian','Tidak dapat dibekalkan'].includes(status))return 'Status pendispensan tidak sah.';if(!items.length)return 'Sekurang-kurangnya satu ubat diperlukan.';for(const item of items){const prescribed=Number(item.kuantitiDipreskripsi),dispensed=Number(item.kuantitiDidispens);if(!cleanText(item.namaUbat||item.name||item.medication,200)||!cleanText(item.dos||item.dose,100)||!cleanText(item.kekerapan||item.frequency,100))return 'Nama ubat, dos dan kekerapan diperlukan.';if(!Number.isInteger(prescribed)||prescribed<1||!Number.isInteger(dispensed)||dispensed<0||dispensed>prescribed)return 'Kuantiti dipreskripsi atau didispens tidak sah.';if(item.expiryDate&&!validDate(item.expiryDate))return 'Tarikh luput ubat tidak sah.';}if(status==='Lengkap'&&items.some(item=>Number(item.kuantitiDidispens)!==Number(item.kuantitiDipreskripsi)))return 'Status Lengkap memerlukan semua kuantiti didispens sama dengan kuantiti dipreskripsi.';if(status==='Sebahagian'&&(!items.some(item=>Number(item.kuantitiDidispens)>0)||!items.some(item=>Number(item.kuantitiDidispens)<Number(item.kuantitiDipreskripsi))))return 'Status Sebahagian memerlukan sekurang-kurangnya satu kuantiti diberi dan satu kekurangan.';if(status==='Tidak dapat dibekalkan'&&items.some(item=>Number(item.kuantitiDidispens)!==0))return 'Status Tidak dapat dibekalkan memerlukan kuantiti didispens sifar.';if(status!=='Lengkap'&&!cleanText(body.incompleteReason,200))return 'Sebab pendispensan tidak lengkap diperlukan.';return '';}
function medicineInventoryValidation(body){const quantity=Number(body.quantity),minimumStock=Number(body.minimumStock??0);if(!cleanText(body.name,200)||!cleanText(body.strength,100)||!cleanText(body.form,100)||!cleanText(body.unit,40)||!cleanText(body.batchNumber,100))return 'Nama, kekuatan, bentuk, unit dan nombor batch diperlukan.';if(!body.expiryDate||!validDate(body.expiryDate))return 'Tarikh luput yang sah diperlukan.';if(!Number.isInteger(quantity)||quantity<0||!Number.isInteger(minimumStock)||minimumStock<0)return 'Kuantiti atau paras stok minimum tidak sah.';return '';}
function applyDispensaryStock(inventory,oldItems,newItems,missionId,recordId,dispenseDate){const next=inventory.map(item=>({...item,stockMovements:Array.isArray(item.stockMovements)?[...item.stockMovements]:[]})),now=new Date().toISOString(),normalized=value=>String(value||'').trim().toLowerCase().replace(/\s+/g,' ');for(const item of oldItems||[]){const quantity=Number(item.kuantitiDidispens||0);if(!quantity||!item.inventoryItemId)continue;const stock=next.find(value=>String(value.id)===String(item.inventoryItemId)&&String(value.missionId)===String(missionId));if(stock){stock.quantity=Number(stock.quantity||0)+quantity;stock.stockMovements.push({id:makeId('MEDMOV'),type:'IN',quantity,balanceAfter:stock.quantity,dispensaryRecordId:recordId,date:now,note:'Pelarasan semula rekod pendispensan'});}}for(const item of newItems||[]){const quantity=Number(item.kuantitiDidispens||0);if(!quantity)continue;if(!item.inventoryItemId)return {error:`Pilih batch inventori untuk ${cleanText(item.namaUbat||item.name,200)||'ubat'}.`};const stock=next.find(value=>String(value.id)===String(item.inventoryItemId)&&String(value.missionId)===String(missionId));if(!stock)return {error:'Batch inventori ubat tidak dijumpai dalam misi ini.'};if(normalized(stock.name)!==normalized(item.namaUbat||item.name))return {error:'Batch inventori tidak sepadan dengan nama ubat yang dipreskripsi.'};if(stock.expiryDate&&String(stock.expiryDate)<String(dispenseDate))return {error:`Batch ${stock.batchNumber} telah luput.`};if(Number(stock.quantity||0)<quantity)return {error:`Stok ${stock.name} batch ${stock.batchNumber} tidak mencukupi.`};stock.quantity=Number(stock.quantity)-quantity;stock.updatedAt=now;stock.stockMovements.push({id:makeId('MEDMOV'),type:'OUT',quantity,balanceAfter:stock.quantity,dispensaryRecordId:recordId,date:now,note:`Didispens kepada pesakit (${recordId})`});item.batchNumber=stock.batchNumber;item.expiryDate=stock.expiryDate;item.inventoryItemId=stock.id;}return {inventory:next};}
app.get('/api/medicine-inventory',async(req,res)=>{let list=await getMedicineInventory();if(req.query.missionId)list=list.filter(item=>String(item.missionId)===String(req.query.missionId));res.json({success:true,data:list,total:list.length});});
app.post('/api/medicine-inventory',async(req,res)=>{const missionId=cleanText(req.body.missionId,100),sourceMedicineId=cleanText(req.body.sourceMedicineId,100),list=await getMedicineInventory(),now=new Date().toISOString();if(missionId&&!(await getMissions()).some(item=>String(item.id)===missionId))return res.status(404).json({success:false,message:'Misi tidak dijumpai.'});let payload={...req.body,missionId:missionId||''},quantity=Number(req.body.quantity),source=null;if(sourceMedicineId){if(!missionId)return res.status(400).json({success:false,message:'Pemindahan stok MyEMT memerlukan ID misi.'});source=list.find(item=>String(item.id)===sourceMedicineId&&!item.missionId);if(!source)return res.status(404).json({success:false,message:'Stok ubat MyEMT tidak dijumpai.'});if(!Number.isInteger(quantity)||quantity<1||quantity>Number(source.quantity))return res.status(409).json({success:false,message:'Kuantiti diminta melebihi baki stok ubat MyEMT.'});payload={...source,...req.body,id:undefined,missionId,sourceMedicineId:source.id,medicineOrigin:'MyEMT',procurementCategory:'Stok Ubat MyEMT',quantity,stockMovements:undefined};}const validation=medicineInventoryValidation(payload);if(validation)return res.status(400).json({success:false,message:validation});const duplicate=list.find(item=>String(item.missionId||'')===String(missionId)&&String(item.batchNumber).toLowerCase()===String(payload.batchNumber).trim().toLowerCase()&&String(item.name).toLowerCase()===String(payload.name).trim().toLowerCase());if(duplicate&&!source)return res.status(409).json({success:false,message:'Ubat dan nombor batch ini telah wujud dalam inventori tersebut.'});if(source){source.quantity=Number(source.quantity)-quantity;source.updatedAt=now;source.stockMovements=[...(source.stockMovements||[]),{id:makeId('MEDMOV'),type:'OUT',quantity,balanceAfter:source.quantity,missionId,date:now,note:`Dipindahkan kepada misi ${missionId}`}];if(duplicate){duplicate.quantity=Number(duplicate.quantity)+quantity;duplicate.updatedAt=now;duplicate.stockMovements=[...(duplicate.stockMovements||[]),{id:makeId('MEDMOV'),type:'IN',quantity,balanceAfter:duplicate.quantity,sourceMedicineId:source.id,date:now,note:'Diterima daripada stok ubat MyEMT'}];await saveMedicineInventory(list);return res.status(201).json({success:true,data:duplicate,message:`Stok dipindahkan. Baki stok MyEMT: ${source.quantity}.`});}}const item={...payload,id:makeId('MED'),quantity,minimumStock:Number(payload.minimumStock||0),createdAt:now,updatedAt:now,stockMovements:[{id:makeId('MEDMOV'),type:'IN',quantity,balanceAfter:quantity,sourceMedicineId:source?.id||'',date:now,note:source?'Diterima daripada stok ubat MyEMT':'Baki pembukaan'}]};list.push(item);await saveMedicineInventory(list);res.status(201).json({success:true,data:item,message:source?`Stok dipindahkan. Baki stok MyEMT: ${source.quantity}.`:'Stok ubat berjaya ditambah.'});});
app.patch('/api/medicine-inventory/:id',async(req,res)=>{const list=await getMedicineInventory(),index=list.findIndex(item=>String(item.id)===String(req.params.id));if(index<0)return res.status(404).json({success:false,message:'Stok ubat tidak dijumpai.'});if(String(req.query.missionId||'')!==String(list[index].missionId||''))return res.status(403).json({success:false,message:'Stok ubat bukan milik skop inventori ini.'});const{id,missionId,createdAt,stockMovements,...updates}=req.body||{},candidate={...list[index],...updates},validation=medicineInventoryValidation(candidate);if(validation)return res.status(400).json({success:false,message:validation});const before=Number(list[index].quantity),after=Number(candidate.quantity),movement=after===before?[]:[{id:makeId('MEDMOV'),type:after>before?'IN':'OUT',quantity:Math.abs(after-before),balanceAfter:after,date:new Date().toISOString(),note:'Pelarasan manual inventori'}];list[index]={...candidate,id:list[index].id,missionId:list[index].missionId,createdAt:list[index].createdAt,updatedAt:new Date().toISOString(),stockMovements:[...(list[index].stockMovements||[]),...movement]};await saveMedicineInventory(list);res.json({success:true,data:list[index],message:'Stok ubat berjaya dikemas kini.'});});
app.get('/api/dispensary-records',async(req,res)=>{let list=await getDispensaryRecords();if(req.query.missionId)list=list.filter(item=>String(item.missionId)===String(req.query.missionId));if(req.query.mdsId)list=list.filter(item=>String(item.mdsId)===String(req.query.mdsId));res.json({success:true,data:list,total:list.length});});
app.get('/api/dispensary-records/:id',async(req,res)=>{const item=(await getDispensaryRecords()).find(value=>String(value.id)===String(req.params.id));if(!item)return res.status(404).json({success:false,message:'Rekod dispensari tidak dijumpai.'});if(req.query.missionId&&String(req.query.missionId)!==String(item.missionId))return res.status(403).json({success:false,message:'Rekod dispensari bukan milik misi ini.'});res.json({success:true,data:item});});
app.post('/api/dispensary-records',async(req,res)=>{const missionId=cleanText(req.body.missionId,100),mdsId=cleanText(req.body.mdsId,100),validation=dispensaryValidation(req.body);if(!missionId||!mdsId)return res.status(400).json({success:false,message:'ID misi dan ID MDS diperlukan.'});if(validation)return res.status(400).json({success:false,message:validation});const mdsList=await getMds(),mdsIndex=mdsList.findIndex(item=>String(item.id||item.mdsRecordId)===mdsId&&String(item.missionId)===missionId),mds=mdsList[mdsIndex];if(!mds)return res.status(409).json({success:false,message:'Rekod MDS tidak sepadan dengan misi.'});const list=await getDispensaryRecords();if(list.some(item=>String(item.missionId)===missionId&&String(item.mdsId)===mdsId))return res.status(409).json({success:false,message:'Rekod MDS ini telah mempunyai rekod pendispensan. Gunakan tindakan Kemas Kini.'});const now=new Date().toISOString(),id=makeId('DSP'),prescriptions=req.body.prescriptions.map(item=>({...item})),stockResult=applyDispensaryStock(await getMedicineInventory(),[],prescriptions,missionId,id,req.body.dispenseDate);if(stockResult.error)return res.status(409).json({success:false,message:stockResult.error});const item={...req.body,prescriptions,id,missionId,mdsId,patientId:mds.patientId||mds.id||mds.mdsRecordId,patientName:mds.patientName||mds.fullName||'',diagnosis:mds.diagnosis||mds.clinicalNotes||mds.diseaseType||mds.traumaType||'',createdAt:now,updatedAt:now};await saveMedicineInventory(stockResult.inventory);list.push(item);await saveDispensaryRecords(list);mdsList[mdsIndex]={...mds,workflowStage:['command_post_verified'].includes(mds.workflowStage)?mds.workflowStage:'pharmacy_completed',recordStatus:mds.workflowStage==='command_post_verified'?mds.recordStatus:'Farmasi Selesai',pharmacyUserId:item.officerId||'',pharmacyUserName:item.officerName,pharmacyCompletedAt:now,updatedAt:now};await saveMds(mdsList);res.status(201).json({success:true,message:'Ubat telah disahkan diberi dan baki stok dikemas kini.',data:item});});
app.patch('/api/dispensary-records/:id',async(req,res)=>{const list=await getDispensaryRecords(),index=list.findIndex(item=>String(item.id)===String(req.params.id));if(index<0)return res.status(404).json({success:false,message:'Rekod dispensari tidak dijumpai.'});if(!req.query.missionId||String(req.query.missionId)!==String(list[index].missionId))return res.status(403).json({success:false,message:'Rekod dispensari bukan milik misi ini.'});const{id,missionId,mdsId,patientId,patientName,diagnosis,createdAt,...updates}=req.body||{},candidate={...list[index],...updates,prescriptions:(updates.prescriptions||list[index].prescriptions).map(item=>({...item}))},validation=dispensaryValidation(candidate);if(validation)return res.status(400).json({success:false,message:validation});const stockResult=applyDispensaryStock(await getMedicineInventory(),list[index].prescriptions,candidate.prescriptions,list[index].missionId,list[index].id,candidate.dispenseDate);if(stockResult.error)return res.status(409).json({success:false,message:stockResult.error});list[index]={...candidate,id:list[index].id,missionId:list[index].missionId,mdsId:list[index].mdsId,patientId:list[index].patientId,patientName:list[index].patientName,diagnosis:list[index].diagnosis,createdAt:list[index].createdAt,updatedAt:new Date().toISOString()};await saveMedicineInventory(stockResult.inventory);await saveDispensaryRecords(list);res.json({success:true,message:'Rekod pendispensan dan baki stok berjaya dikemas kini.',data:list[index]});});
...
async function validateOperationalAsset(body,assetList){
  if(isMedicineAsset(body))return 'Ubat mesti direkodkan dalam modul Inventori Ubat, bukan Inventori Aset.';
...
for(const resource of ['assets','storage','transport','stores']){
  app.get(`/api/${resource}/summary`,async(req,res)=>{let list=await getOperationalData(resource);if(resource==='assets')list=list.filter(item=>!isMedicineAsset(item));const statusOf=item=>item.currentStatus||item.status,inventory=list.filter(item=>!item.missionId),minimumOf=item=>item.minimumStockLevel===undefined||item.minimumStockLevel===null||item.minimumStockLevel===''?2:Number(item.minimumStockLevel);res.json({success:true,total:list.length,active:list.filter(item=>['Aktif','Tersedia','Baik','Sedia Digunakan'].includes(statusOf(item))).length,maintenance:list.filter(item=>statusOf(item)==='Penyelenggaraan').length,critical:list.filter(item=>['Rosak','Servis Diperlukan'].includes(statusOf(item))).length,lowStock:resource==='assets'?inventory.filter(item=>Number(item.quantity||0)<=minimumOf(item)).length:0,outOfStock:resource==='assets'?inventory.filter(item=>Number(item.quantity||0)===0).length:0,byCategory:list.reduce((all,item)=>{const key=item.category||'Tidak Dinyatakan';all[key]=(all[key]||0)+1;return all;},{}),byStatus:list.reduce((all,item)=>{const key=statusOf(item)||'Tidak Dinyatakan';all[key]=(all[key]||0)+1;return all;},{})});});
  app.get(`/api/${resource}`,async(req,res)=>{let list=await getOperationalData(resource);if(resource==='assets')list=list.filter(item=>!isMedicineAsset(item));if(req.query.missionId)list=list.filter(item=>String(item.missionId)===String(req.query.missionId));res.json({success:true,data:list,total:list.length});});
  app.get(`/api/${resource}/:id`,async(req,res)=>{const item=(await getOperationalData(resource)).find(value=>String(value.id)===String(req.params.id));if(!item)return res.status(404).json({success:false,message:'Rekod tidak dijumpai.'});const requestedMissionId=cleanText(req.query.missionId,100),ownerMissionId=cleanText(item.missionId,100);if(resource==='assets'){if(ownerMissionId&&requestedMissionId!==ownerMissionId)return res.status(403).json({success:false,message:'Aset ini dimiliki oleh misi dan mesti dibuka melalui halaman butiran misi.'});if(!ownerMissionId&&requestedMissionId)return res.status(409).json({success:false,message:'Aset inventori MyEMT tidak dimiliki oleh misi ini.'});}else if((resource==='transport'||resource==='storage')&&requestedMissionId!==ownerMissionId)return res.status(403).json({success:false,message:ownerMissionId?'Rekod logistik ini hanya boleh dibuka melalui misi pemiliknya.':'Rekod logistik ini ialah milik pusat MyEMT.'});res.json({success:true,data:item});});
  app.post(`/api/${resource}`,async(req,res)=>{
    ...
    if(resource==='assets'){
      ...
      if(resource==='stores'){const storeError=validateStoreLocation(payload);if(storeError)return res.status(400).json({success:false,message:storeError});if(list.some(item=>String(item.code||'').toLowerCase()===String(payload.code||'').toLowerCase()))return res.status(409).json({success:false,message:'Kod lokasi stor telah digunakan.'});}
      if(resource==='transport'&&list.some(item=>String(item.missionId||'')===missionId&&String(item.registrationNumber||item.plate||'').toLowerCase()===String(payload.registrationNumber||'').toLowerCase()))return res.status(409).json({success:false,message:'Nombor pendaftaran ini telah digunakan dalam misi tersebut.'});
      if(resource==='storage'&&payload.serialNumber&&list.some(item=>String(item.missionId||'')===missionId&&String(item.serialNumber||'').toLowerCase()===String(payload.serialNumber).toLowerCase()))return res.status(409).json({success:false,message:'Nombor siri penyimpanan ini telah digunakan dalam misi tersebut.'});
    }
    const item={...payload,...(missionId?{missionId}:{missionId:null}),id:makeId(resource.slice(0,2).toUpperCase()),createdAt:now,updatedAt:now};
  ...
  app.patch(`/api/${resource}/:id`,async(req,res)=>{const list=await getOperationalData(resource),index=list.findIndex(value=>String(value.id)===String(req.params.id));if(index<0)return res.status(404).json({success:false,message:'Rekod tidak dijumpai.'});const{id,missionId,createdAt,stockMovements,...updates}=req.body||{};const candidate={...list[index],...updates};const requestedMissionId=cleanText(req.query.missionId,100),ownerMissionId=cleanText(list[index].missionId,100);if((resource==='transport'||resource==='storage')&&requestedMissionId!==ownerMissionId)return res.status(403).json({success:false,message:ownerMissionId?'Rekod logistik ini hanya boleh dikemas kini melalui misi pemiliknya.':'Rekod logistik pusat MyEMT tidak boleh dikemas kini sebagai rekod misi.'});if(resource==='transport'||resource==='storage'){const logisticsError=validateMissionLogistics(resource,candidate);if(logisticsError)return res.status(400).json({success:false,message:logisticsError});if(resource==='transport'&&list.some((item,itemIndex)=>itemIndex!==index&&String(item.missionId||'')===ownerMissionId&&String(item.registrationNumber||item.plate||'').toLowerCase()===String(candidate.registrationNumber||candidate.plate||'').toLowerCase()))return res.status(409).json({success:false,message:'Nombor pendaftaran ini telah digunakan dalam skop pemilikan tersebut.'});if(resource==='storage'&&candidate.serialNumber&&list.some((item,itemIndex)=>itemIndex!==index&&String(item.missionId||'')===ownerMissionId&&String(item.serialNumber||'').toLowerCase()===String(candidate.serialNumber).toLowerCase()))return res.status(409).json({success:false,message:'Nombor siri penyimpanan ini telah digunakan dalam skop pemilikan tersebut.'});}if(resource==='stores'){const storeError=validateStoreLocation(candidate);if(storeError)return res.status(400).json({success:false,message:storeError});if(list.some((item,itemIndex)=>itemIndex!==index&&String(item.code||'').toLowerCase()===String(candidate.code||'').toLowerCase()))return res.status(409).json({success:false,message:'Kod lokasi stor telah digunakan.'});}if(resource==='assets'){if(ownerMissionId&&requestedMissionId!==ownerMissionId)return res.status(403).json({success:false,message:'Aset ini dimiliki oleh misi dan hanya boleh dikemas kini melalui halaman butiran misi.'});if(!ownerMissionId&&requestedMissionId)return res.status(409).json({success:false,message:'Aset inventori MyEMT tidak dimiliki oleh misi ini.'});const assetError=await validateOperationalAsset(candidate);if(assetError)return res.status(400).json({success:false,message:assetError});candidate.status=candidate.currentStatus||candidate.status;if(!ownerMissionId&&updates.quantity!==undefined){const before=Number(list[index].quantity||0),after=Number(updates.quantity);if(!Number.isInteger(after)||after<0)return res.status(400).json({success:false,message:'Kuantiti inventori tidak sah.'});const difference=after-before;if(difference)candidate.stockMovements=[...(Array.isArray(list[index].stockMovements)?list[index].stockMovements:[]),{id:makeId('MOV'),type:difference>0?'IN':'OUT',quantity:Math.abs(difference),balanceAfter:after,date:new Date().toISOString(),note:'Pelarasan stok inventori'}];}}list[index]={...candidate,id:list[index].id,missionId:list[index].missionId||null,createdAt:list[index].createdAt,updatedAt:new Date().toISOString()};await saveOperationalData(resource,list);res.json({success:true,data:list[index],message:'Rekod berjaya dikemas kini.'});});
}
